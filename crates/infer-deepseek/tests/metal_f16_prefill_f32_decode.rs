...
fn decode_step(
    ...
    model
        .language_model()
        .forward(None, Some(&embed), None, None, Some(cache), true)?;
    Ok(())
}

#[test]
#[ignore]
fn metal_f16_prefill_f32_decode() -> Result<()> {
    ...
    let token_embeddings = model.language_model().embed_tokens(&prefill_ids)?;
    let injected = model.inject_image_tokens_for_tests(
        token_embeddings,
        &prefill_mask,
        Some(image_embeddings.as_slice()),
    )?;
    let prefill_embeddings_f32 = injected.to_dtype(DType::F32)?;
    ...
    let mut cache_f32 = model.new_cache_for_dtype(DType::F32)?;
    cache_f32.clear();
    model.language_model().forward(
        None,
        Some(&prefill_embeddings_f32),
        None,
        None,
        Some(&mut cache_f32),
        true,
    ...
    let out_f32 = model.language_model().forward(
        None,
        Some(&embed.to_dtype(DType::F32)?),
        None,
        None,
    ...
    println!(
        "f16 prefill+decode: top1 {t1} {v1} top2 {t2} {v2} margin {}",
        v1 - v2
    );
    let (t1, v1, t2, v2) = top2_logits(&logits_f32)?;
    println!(
        "f32 prefill cache: top1 {t1} {v1} top2 {t2} {v2} margin {}",
        v1 - v2
    );
